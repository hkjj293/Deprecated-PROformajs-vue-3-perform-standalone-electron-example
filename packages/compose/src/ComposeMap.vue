...
import { TemporalArrow } from '@openclinical/proformajs-vue3-map'
import { PathBreadcrumb } from '@openclinical/proformajs-vue3-map'
...
export default {
  ...
  components: {
    'c-maptask': ComposeMapTask,
    'm-breadcrumb': PathBreadcrumb,
    'm-taskbar': MapTaskBar,
    'm-arrow': TemporalArrow
  },
  computed: {
    plan() {
      let plan
    ...
    viewbox() {
      return (
        '0 0 ' +
        (this.protocol.meta.svg.width + this.taskbar.width) +
        ' ' +
    ...
    ports() {
      ...
      if (this.plan && this.plan.tasks) {
        for (let task of this.plan.tasks) {
          result[task.name] = {
            to: {
              x: task.meta.pos.x + this.taskbar.width,
              y: task.meta.pos.y + this.breadcrumb.height
            }
          }
          result[task.name].from = {
            x: task.meta.pos.x + this.taskbar.width + 40,
            y: task.meta.pos.y + this.breadcrumb.height
          }
          // small adjustments
          if (task.constructor.name == 'Plan') {
            ...
            result[task.name].from.x =
              task.meta.pos.x + this.taskbar.width + 40 - this.stroke_width / 2
  ...
  methods: {
    ...
    handleKey(evt) {
      if (evt.key == 'Delete') {
        if (this.selectedarrow) {
          this.removeAntecedent(
            this.plan.path(),
            this.selectedarrow.source,
            this.selectedarrow.target
          )
          this.$emit('change-protocol', { value: this.protocol, emitter: 'c-map.0' })
    ...
    deleteTask() {
      ...
      this.$emit('change-protocol', { value: this.protocol, emitter: 'c-map.1' })
    },
    ...
    arrowSelected(dep) {
      if (this.selectedarrow) {
        return (
          this.selectedarrow.target == dep.target.name &&
          this.selectedarrow.source == dep.source.name
        )
      } else {
        return false
      }
    ...
    startDrag(evt) {
      ...
      if (evt.target.parentNode && evt.target.parentNode.classList.contains('draggable')) {
        ...
        this.selectedarrow = {
          source: evt.target.parentNode.dataset.source,
          target: evt.target.parentNode.dataset.target
        }
      } else {
        // unselect arrow
    ...
    drag(evt) {
      ...
      if (selectedElement) {
        ...
        if (selectedElement.dataset.path) {
          ...
          this.$emit('change-protocol', { value: this.protocol, emitter: 'c-map.2' })
        } else {
        ...
        if (!selectedElement.dataset.path) {
          ...
          if (clazz) {
            ...
            if (coord.x - offset.x + this.taskbar.layout[clazz].x > this.taskbar.width) {
              task.meta.pos.x =
                coord.x - offset.x + this.taskbar.layout[clazz].x - this.taskbar.width
              task.meta.pos.y =
                coord.y - offset.y + this.taskbar.layout[clazz].y - this.breadcrumb.height
              try {
                this.plan.addTask(task)
                this.$emit('change-protocol', { value: this.protocol, emitter: 'c-map.3' })
        ...
        for (let task of this.plan.tasks) {
          if (task.name != this.dragArrow.source) {
            ...
            if (
              ...
              this.$emit('change-protocol', { value: this.protocol, emitter: 'c-map.4' })
            }
          }
        }
        this.dragArrow.show = false
